...
# ── JILA / Ana Maria Rey (Optical Lattice MBL) ───────────────────────
# Real data: Zenodo repository (e.g., Strontium spin-exchange dynamics)
# Fallback:  Synthetic Many-Body Localization in 1D lattice
JILA_ZENODO_NOTE = """
  [JILA] Ana Maria Rey / Jun Ye Lab Experimental Data:
    1. Go to Zenodo and search for 'Strontium optical lattice spin-exchange dynamics' 
       or use a direct DOI (e.g., 10.5281/zenodo.xxxxxxx)
    2. Download the supplementary .mat or .csv files (Time-resolved spin populations)
    3. Place the files in ./data/jila_mbl/
"""

def fetch_jila_mbl():
    out = DATA_DIR / "jila_mbl"
    real_out = out / "real"
    out.mkdir(parents=True, exist_ok=True)
    real_out.mkdir(parents=True, exist_ok=True)
    
    print("  [JILA] Downloading real Moire Lattice MBL data from Zenodo (10.5281/zenodo.15337680)...")
    ...
    try:
        ...
        with zipfile.ZipFile(io.BytesIO(response.read())) as z:
            for file_info in z.infolist():
                if file_info.filename.startswith("fig2_data/imbalanceTimeDecay") and file_info.filename.endswith(".txt"):
                    z.extract(file_info, real_out)
        print(f"  [JILA] Successfully extracted real data to {real_out}")
        return # Skip synthetic generation if successful
    except Exception as e:
        print(f"  [JILA] Failed to download real Zenodo data: {e}")
        print("  [JILA] Generating synthetic MBL spin-exchange data (Ana Maria Rey model fallback)...")

    # Generate synthetic MBL data as fallback
    rng = np.random.default_rng(42)
    n_sites = 32
    T_steps = 128
    
    # In MBL, disorder prevents thermalization.
    # Spin population <S_z> remains localized near initial state.
    # We simulate strong disorder (W > W_c) with weak oscillatory exchange.
    data = np.zeros((T_steps, n_sites, n_sites))
    
    # Random initial localized state (e.g., Neel state or random domain walls)
    initial_spins = rng.choice([-1.0, 1.0], size=n_sites)
    
    # Disorder potential
    disorder = rng.uniform(-5.0, 5.0, size=n_sites)
    
    for t in range(T_steps):
        dt = 0.1 * t
        # Spins precess locally due to disorder, with very weak nearest-neighbor hopping
        for i in range(n_sites):
            # The signal remains strongly correlated with its initial position (Localization)
            # Damping is near zero (gamma_k -> 0), phase velocity depends on disorder (omega_k -> disorder)
            oscillation = np.cos(disorder[i] * dt)
            # Add small many-body dephasing
            dephasing = np.exp(-0.01 * dt)
            data[t, i, i] = initial_spins[i] * oscillation * dephasing + rng.normal(0, 0.05)
            
    path = out / "mbl_spin_dynamics.npy"
    np.save(path, data)
    print(f"  [JILA] Saved synthetic MBL data → {path}")
...
if __name__ == "__main__":
    ...
    p.add_argument("--jila",         action="store_true")
    args = p.parse_args()
