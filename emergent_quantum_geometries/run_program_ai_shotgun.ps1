# run_program_ai_shotgun.ps1  --  Program AI TPU Orchestrator
# Project: time-emission
# Target: emergent_quantum_geometries/program_ai_predictive_hydrodynamics.py

$Project   = "time-emission"
$SrcDir    = "$PSScriptRoot"
$RemoteDir = "/home/cityz/program_ai"
$LogFile   = "prog_ai_run.log"
...
$QRName  = "program-ai-qr-v6"
$NodeId  = "program-ai-node-v6"
$Zone    = "us-east1-d"
$Type    = "v6e-8"
...
if ($qrState -match "MISSING|FAILED|SUSPENDED") {
    ...
    # It was already queued, we just need to poll
...
Write-Host "[EXECUTE] Launching Flagship Latent Semantic Flow Fields experiment..."
$RunCmd = "export OMP_NUM_THREADS=64; nohup python3 -u $RemoteDir/emergent_quantum_geometries/program_ai_predictive_hydrodynamics.py --require-tpu > $RemoteDir/$LogFile 2>&1 &"
"y" | gcloud compute tpus tpu-vm ssh $NodeId `
    --project=$Project --zone=$Zone `
    --command=$RunCmd
...
while ((Get-Date) -lt $RunTimeout) {
    ...
    $doneRaw = "y" | gcloud compute tpus tpu-vm ssh $NodeId `
        --project=$Project --zone=$Zone `
        --command="grep -c 'Summary master printout' $RemoteDir/$LogFile 2>/dev/null || echo 0" 2>$null
    ...
    $isRunning = "y" | gcloud compute tpus tpu-vm ssh $NodeId `
        --project=$Project --zone=$Zone `
        --command="pgrep -f program_ai_predictive_hydrodynamics.py >/dev/null && echo 1 || echo 0" 2>$null
...
$remoteResult = "$NodeId`:$RemoteDir`/emergent_quantum_geometries`/program_ai_results`/program_ai_summary.json"
$remoteLog    = "$NodeId`:$RemoteDir`/$LogFile"

New-Item -ItemType Directory -Force -Path "$SrcDir\program_ai_results" | Out-Null
"y" | gcloud compute tpus tpu-vm scp $remoteResult "$SrcDir\program_ai_results\program_ai_summary.json" --project=$Project --zone=$Zone --quiet 2>$null
"y" | gcloud compute tpus tpu-vm scp $remoteLog "$SrcDir\program_ai_results\program_ai_run.log" --project=$Project --zone=$Zone --quiet 2>$null
