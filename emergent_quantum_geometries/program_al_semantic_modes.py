"""
Program AJ — Forecast Horizon & Stability Phase Diagram
=======================================================================
Extends Program AI to map the stability boundaries of the predictive
controller by sweeping the Koopman forecast horizon (k).
Features: Incremental JSON saves, Watchdog preemptions, Latent exports.
"""
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=2) as response:
                if response.read().decode('utf-8') == "TRUE":
                    print("[WATCHDOG] Preemption imminent! Triggering graceful shutdown!")
                    SHUTDOWN_FLAG = True
        except:
            pass
        time.sleep(5)
...
from program_ag_tpu import get_gell_mann_matrices, density_to_bloch, build_compositional_motifs, vmap_free_global, vmap_reinject_global
import scipy.linalg as la

def get_cached_eigh_fast(H, cache_key):
    print("  [CPU/MKL] Diagonalizing Hamiltonian via multi-threaded scipy.linalg.eigh...")
    evals, evecs = la.eigh(np.array(H, dtype=np.complex64))
    return evals, evecs
...
def compute_recurrence_score(z_coords):
    # Detect persistent cycles/loops by measuring minimum non-local return distance
    if z_coords.ndim == 3:
        # z_coords: (n_traj, T, d)
        diffs = z_coords[:, :, None, :] - z_coords[:, None, :, :]
        dmat = jnp.sum(diffs**2, axis=-1)
        T_steps = dmat.shape[-1]
        mask = jnp.abs(jnp.arange(T_steps)[:, None] - jnp.arange(T_steps)[None, :]) > 10
        dmat = jnp.where(mask, dmat, 1e9)
        return float(jnp.mean(jnp.min(dmat, axis=-1)))
    else:
        diffs = z_coords[:, None, :] - z_coords[None, :, :]
        dmat = jnp.sum(diffs**2, axis=-1)
        T_steps = dmat.shape[1]
...
def fit_koopman_operator(z_train):
    ...
    K_op = K.T  # (d, d)
    # JAX TPU does not support non-Hermitian 'eig'. Force CPU via numpy.
    evals, evecs = np.linalg.eig(np.array(K_op))
    return K_op, evals, evecs
...
def apply_matrices_inverse(psi, matrices, indices, N):
    for U, (i, j) in zip(reversed(matrices), reversed(indices)):
        psi = apply_2q_gate(psi, U.conj().T, i, j, N)
    return psi

def extract_single_rho(psi, bob_matrices, bob_indices, N):
    psi_dec = apply_matrices_inverse(psi, bob_matrices, bob_indices, N)
...
def make_predictive_runner(block_pairs, N, n_steps, bob_indices, lambdas_j, d_hidden):
    def _run(U_dt, dt, p1, p2, p_cross, T_quiet, eps_predictive, forecast_horizon, gate_budget, psi0, rng_key, K, rnn_params, bob_matrices):
        
        S0 = compute_block_entropy(psi0, block_pairs[0][0], block_pairs[0][1], N)
        rho0 = extract_single_rho(psi0, bob_matrices, bob_indices, N)
        z0 = density_to_bloch(rho0, lambdas_j)
        h0 = jnp.zeros(d_hidden)
        
        carry0 = (psi0, S0, jnp.int32(999), jnp.int32(0), rng_key, z0, h0, gate_budget)
        
        def step(carry, s):
            psi, S_prev, ssl, ng, key, z_prev, h_prev, rem_budget = carry
            key, _, ks = jax.random.split(key, 3)
            
            def forecast_step(i, carry):
                h_i, z_i = carry
                h_n, r_n = rnn_forward(rnn_params, h_i, z_i)
                z_n = z_i @ K.T + r_n
                return (h_n, z_n)
            
            h_next, z_pred = jax.lax.fori_loop(0, forecast_horizon, forecast_step, (h_prev, z_prev))
            # Step recurrence correctly using actual 1-step logic
            h_next_1, _ = rnn_forward(rnn_params, h_prev, z_prev)
            ...
            # Sparse Scheduling: Check budget limits
            has_budget = jnp.logical_or(gate_budget < 0, rem_budget > 0)
            trigger = jnp.logical_and(trigger, has_budget)
            
            gi = jax.lax.cond(trigger, lambda: jnp.array((3, 0)), lambda: jnp.array((0, 0)))
            ci = gi[0]
            
            psi = run_step_psi(psi, None, U_dt, N, dt, p1, p2, p_cross, gi, ks)
            
            ssl_n = jax.lax.cond(ci > 0, lambda: jnp.int32(0), lambda: ssl + 1)
            ng_n = ng + jnp.where(ci > 0, 1, 0)
            rem_budget_n = rem_budget - jnp.where(ci > 0, 1, 0)
            fired = jnp.where(ci > 0, 1.0, 0.0)
            
            rho_curr = extract_single_rho(psi, bob_matrices, bob_indices, N)
            z_curr = density_to_bloch(rho_curr, lambdas_j)
            S_curr = compute_block_entropy(psi, block_pairs[0][0], block_pairs[0][1], N)
            
            return (psi, S_curr, ssl_n, ng_n, key, z_curr, h_next_1, rem_budget_n), (fired, pred_drift)
            
        (psi_f, _, _, ng_f, _, _, _, _), (fired, drifts) = jax.lax.scan(step, carry0, jnp.arange(n_steps, dtype=jnp.int32))
        return psi_f, ng_f, fired, drifts
        
    return jax.jit(jax.vmap(_run, in_axes=(None, None, None, None, None, None, None, None, None, 0, 0, None, None, None)))

def make_reactive_runner(block_pairs, N, n_steps):
    def _run(U_dt, dt, p1, p2, p_cross, T_quiet, eps_S, psi0, rng_key):
        S0 = compute_block_entropy(psi0, block_pairs[0][0], block_pairs[0][1], N)
    ...
    return jax.jit(jax.vmap(_run, in_axes=(None, None, None, None, None, None, None, 0, 0)))

# ---------------------------------------------------------------------------
# Master Execution
# ---------------------------------------------------------------------------

def run_program_aj(args):
    global SHUTDOWN_FLAG
    t0 = time.time()
    select_backend("jax", require_tpu=getattr(args, "require_tpu", False))

    print("=" * 80)
    print("Program AL — Semantic Normal Modes in MERA Flow Fields")
    print("=" * 80)
    ...
    summary_path = os.path.join(args.out_dir, "program_al_summary.json")
    all_records = []
    ...
    for depth in args.depth_sweep:
        ...
        # Get topology for this depth (indices are seed-independent)
        _, sample_bob_gates = build_mera_gates_z(depth=depth, seed=0)
        bob_indices = tuple((int(g[1]), int(g[2])) for g in sample_bob_gates)

        def vmap_free(U_dt, N, dt, n_steps, p1, p2, p_cross, psi0, rng_key):
            def step(carry, _):
                psi, key = carry
                key, _, ks = jax.random.split(key, 3)
                psi = run_step_psi(psi, None, U_dt, N, dt, p1, p2, p_cross, jnp.array((0, 0)), ks)
                return (psi, key), None
        ...
        pmap_free = jax.pmap(jax.jit(jax.vmap(vmap_free, in_axes=(None, None, None, None, None, None, None, 0, 0)), static_argnums=(1, 3)), in_axes=(None, None, None, None, None, None, None, 0, 0), static_broadcasted_argnums=(1, 3))
        
        vmap_reactive = make_reactive_runner(ent_block_pairs, N, args.n_steps_per_sym)
        pmap_reactive = jax.pmap(vmap_reactive, in_axes=(None, None, None, None, None, None, None, 0, 0))
        
        d_hidden = 16
        vmap_predictive = make_predictive_runner(ent_block_pairs, N, args.n_steps_per_sym, bob_indices, lambdas_j, d_hidden)
        pmap_predictive = jax.pmap(vmap_predictive, in_axes=(None, None, None, None, None, None, None, None, None, 0, 0, None, None, None))
        ...
        for seed in seeds:
            ...
            evals_train, evecs_train = get_cached_eigh_fast(H_train, (seed, args.j_std_train))
            evals_test, evecs_test = get_cached_eigh_fast(H_test, (seed + 1000, args.j_std_test))
            
            # Precompute U_dt for train
            evals_h = jnp.array(evals_train, dtype=jnp.float32)
            evecs_h = jnp.array(evecs_train, dtype=jnp.complex64)
            U_dt_h = evecs_h @ (jnp.exp(-1j * evals_h * jnp.float32(dt_step))[:, None] * evecs_h.conj().T)
            U_dt_h = jax.device_put(U_dt_h)

            alice_gates, bob_gates = build_mera_gates_z(depth=depth, seed=seed)
            bob_matrices = jnp.stack([g[0] for g in bob_gates])
            ...
            for sl in seq_lengths:
                ...
                def run_ensembles_parallel(evals_h, evecs_h, ctrl_type="free", k_horiz=1, eps_pred=0.015, K=None, rnn_params=None):
                    rhos_all = []
                    gates_all = []
                        ...
                        for sym_t in range(sl):
                            ...
                            if ctrl_type == "free":
                                ...
                                psi_batch, ng, *_ = pmap_predictive(
                                    U_dt_h, jnp.float32(dt_step),
                                    jnp.float32(p1), jnp.float32(p2), jnp.float32(p_cross),
                                    jnp.int32(args.T_quiet), jnp.float32(eps_pred),
                                    jnp.int32(k_horiz), jnp.int32(args.gate_budget),
                                    psi_batch, traj_keys_batch, K, rnn_params, bob_matrices)
                ...
                for ctrl in ["free", "reactive_entropy", "predictive_entropy"]:
                    ...
                    eps_list = args.eps_sweep if ctrl == "predictive_entropy" else [0.0]
                    for k_horiz in k_list:
                        for eps_val in eps_list:
                            ctrl_name = ctrl if ctrl != "predictive_entropy" else f"predictive_k{k_horiz}_e{eps_val}"
                            if ctrl_name not in depth_results:
                                for sl2 in seq_lengths:
                                    depth_results[ctrl_name] = {sl2: {"latent_diff": [], "lyapunov": [], "ricci_contraction": [], "transport_coherence": [], "recurrence_score": [], "mean_gates": []} for sl2 in seq_lengths}
                            ...
                            if os.path.exists(npz_path):
                                ...
                                try:
                                    ...
                                    for ens_idx in range(args.n_ensembles):
                                        ...
                                        recurrence = compute_recurrence_score(z_test)
                                        ...
                                        depth_results[ctrl_name][sl]["recurrence_score"].append(recurrence)
                                    ...
                                    to_run.append((ctrl, k_horiz, eps_val, ctrl_name, npz_path))
                            else:
                                to_run.append((ctrl, k_horiz, eps_val, ctrl_name, npz_path))
                ...
                if len(to_run) > 0:
                    ...
                    K, K_evals, K_evecs = fit_koopman_operator(z_train)
                    rnn_params = train_residual_rnn(z_train, K, epochs=200)

                    for ctrl, k_horiz, eps_val, ctrl_name, npz_path in to_run:
                        print(f"  [Pass 2] Executing {ctrl_name} on Test set...")
                        test_rhos, test_gates = run_ensembles_parallel(
                            evals_test, evecs_test, ctrl_type=ctrl, k_horiz=k_horiz, eps_pred=eps_val, K=K, rnn_params=rnn_params)
                        ...
                        try:
                            np.savez_compressed(npz_path, z_coords=np.array(z_test_all), K_matrix=np.array(K), K_evals=np.array(K_evals), K_evecs=np.array(K_evecs), gates=np.array(test_gates))
                        except Exception as e:
                            print(f"Failed to export npz: {e}")
                            
                        # Incremental JSON save
                        all_records_temp = all_records.copy()
                        rec_temp = {"mera_depth": depth, "gamma": gamma, "seq_lengths": {str(sl): {}}}
                        for c_key in depth_results.keys():
                            rec_temp["seq_lengths"][str(sl)][c_key] = {}
                            for m_key, v_list in depth_results[c_key][sl].items():
                                if len(v_list) > 0:
                                    mean_v = float(np.mean(v_list))
                                    ci95 = float(1.96 * np.std(v_list) / np.sqrt(len(v_list))) if len(v_list) > 1 else 0.0
                                    rec_temp["seq_lengths"][str(sl)][c_key][m_key] = {"mean": mean_v, "ci95": ci95}
                        
                        try:
                            with open(summary_path, "w") as f:
                                json.dump({"experiment": "predictive_threshold_sweep", "records": all_records_temp + [rec_temp], "runtime_s": round(time.time() - t0, 1)}, f, indent=2)
                        except:
                            pass
                        ...
                        print(f"    {ctrl_name:>18} -> Diff:{float(np.mean(depth_results[ctrl_name][sl]['latent_diff'])):.4f} "
                              f"  Lyap:{float(np.mean(depth_results[ctrl_name][sl]['lyapunov'])):.4f} "
                              f"  Ricci:{float(np.mean(depth_results[ctrl_name][sl]['ricci_contraction'])):.4f} "
                              f"  Coherence:{float(np.mean(depth_results[ctrl_name][sl]['transport_coherence'])):.3f} "
                              f"  Recurrence:{float(np.mean(depth_results[ctrl_name][sl]['recurrence_score'])):.3f}")

        # Compile final depth record
        record = {"mera_depth": depth, "gamma": gamma, "seq_lengths": {}}
        for sl in seq_lengths:
            record["seq_lengths"][str(sl)] = {}
            for ctrl_name in depth_results.keys():
                record["seq_lengths"][str(sl)][ctrl_name] = {}
                for metric, vals in depth_results[ctrl_name][sl].items():
                    ...
                    record["seq_lengths"][str(sl)][ctrl_name][metric] = {
                        "mean": mean_val,
                        "ci95": ci95_val
                    }
    ...
    out = {
        "experiment": "semantic_modes_sparse_budget",
        "seed_suite": seeds,
        "records": all_records,
    ...
    print("\n" + "=" * 120)
    print(f"  {'d':>2} {'L':>3} {'ctrl':>18} {'Diff':>9} {'Lyap':>9} {'Ricci':>9} {'Recurr':>10} {'Gates':>8}")
    print("-" * 120)
    for r in all_records:
        d = r["mera_depth"]
        for sl in seq_lengths:
            for ctrl_name in sorted(r["seq_lengths"][str(sl)].keys()):
                m = r["seq_lengths"][str(sl)][ctrl_name]
                print(f"  {d:>2} {sl:>3} {ctrl_name:>18} {m['latent_diff']['mean']:>9.4f} {m['lyapunov']['mean']:>9.4f} "
                      f"{m['ricci_contraction']['mean']:>9.4f} {m['recurrence_score']['mean']:>10.3f} {m['mean_gates']['mean']:>8.1f}")
    print("=" * 120 + "\n")
...
if __name__ == "__main__":
    ...
    p.add_argument("--out-dir", default="program_al_results")
    p.add_argument("--require-tpu", action="store_true")
    p.add_argument("--smoke-test", action="store_true")
    p.add_argument("--sequence-lengths", nargs="+", type=int, default=[32])
    p.add_argument("--depth-sweep", nargs="+", type=int, default=[3, 4])
    p.add_argument("--k-horizons", nargs="+", type=int, default=[1, 8])
    p.add_argument("--eps-sweep", nargs="+", type=float, default=[0.1, 0.15, 0.2])
    p.add_argument("--gate-budget", type=int, default=-1, help="-1 for unlimited, or fix a sparse budget")
    args = p.parse_args()
