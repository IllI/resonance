...
We establish an exact theorem relating the teleportation fidelity of two-qubit boundary states generated by the one-axis twisting (OAT) Hamiltonian to their concurrence. The proof proceeds by showing that OAT boundary states form a constrained X-state family with equal diagonal elements, for which the Wootters concurrence and maximal singlet fraction collapse onto a single scalar parameter. The resulting identity $F_\mathrm{opt} = (2+C)/3$ is exact for all system sizes $N$, all coupling times $\chi t$, and all OAT boundary states — not an approximation.

We complement the theorem with: (i) a closed-form exact solution for the two-qubit Lindblad dephasing channel calibrated to $T_2 = 118(9)$ s [Kim *et al.*, 2025]; (ii) a decoherence scaling law $\Gamma^*(N) = 0.550\,N^{-1.03}$ s$^{-1}$ giving a 15.8$\times$ safety margin at $N=4$; (iii) a local phase-alignment rotation protocol that improves fidelity by $(1-C)/6$ per measurement shot using only two $R_z$ gates; and (iv) a witness-based measurement protocol for certifying entanglement without Bell inequality violation, which is the appropriate diagnostic for $N \ge 4$ OAT boundary states.
...
The threshold rate $\Gamma^*(N)$ is where peak fidelity drops to $2/3$. From exact MPS sweeps over $N = 2$ to $16$ and $\Gamma$ from $10^{-4}$ to $1$ s$^{-1}$:
...
Quantum advantage at $N = 4$ requires $\Gamma_\mathrm{eff} = \Gamma_1 + \Gamma_\mathrm{mb} < 0.134$ s$^{-1}$, i.e., many-body contribution $\Gamma_\mathrm{mb} < 0.126$ s$^{-1}$, equivalently superexchange $J_\mathrm{ex} < 4\Gamma_\mathrm{mb} < 0.50$ s$^{-1} \approx 80$ mHz.
...
| 5 | Streaming | 280 shots $\times$ 20 points $= 5600$ total shots |
...
3. **Theorem restricted to OAT X-state family.** The identity $F_\mathrm{opt}=(2+C)/3$ is exact for OAT boundary states; it does not hold for generic mixed states.
...
**Exact results [Exact]:**
- OAT boundary states form a constrained equal-diagonal X-state family for all $N$ and $\chi t$ (Proposition 1).
- The Wootters concurrence reduces to $C = 2\max(0, r - 1/4)$ for this family (Sec. II.C).
- The teleportation fidelity identity $F_\mathrm{opt} = (2+C)/3$ is exact (Theorem, Sec. II.C).
...
- Decoherence scaling law $\Gamma^*(N) = 0.550\,N^{-1.03}$ s$^{-1}$; 15.8$\times$ margin at $N=4$.
