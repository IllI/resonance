...
Open-system analysis under single-particle Lindblad dephasing, anchored to $T_2 = 118$ s, shows quantum advantage ($F > 2/3$) survives with safety margins of $4\times$--$30\times$ over $\Gamma_1 = T_2^{-1}$ for $N = 2$--$16$.
...
This means JILA cannot certify boundary entanglement via a Bell test. However, the linear entanglement witness $W = \frac{1}{4}\mathbb{I} - |\Psi^+\rangle\langle\Psi^+|$, whose expectation value $\mathrm{Tr}[W\rho_2] < 0$ constitutes a certificate of entanglement that does not require Bell inequality violation, provides the correct diagnostic. Our controller is designed around this witness.
...
**Numerical validation.** We compute $\rho_2$ from both the MPS and the exact full-statevector for $N = 2$--$16$ and find
...
where the factor of 4 (not 16) arises from the fact that $W$ selects $f(\Psi^+)$, which involves all four off-diagonal elements with the same Hamming distance structure. Numerical confirmation: D-LinOSS returns $b = \gamma_{k,\mathrm{dom}} = 4\Gamma$ with $|b - 4\Gamma| < 0.001\Gamma$ for all $N = 2$--$16$.

### IV.C Threshold Dephasing Rate and Safety Margins

The threshold rate $\Gamma^*(N)$ is defined as the single-particle dephasing rate at which the peak fidelity drops exactly to $2/3$. Sweeping $N = 2$--$16$ and $\Gamma = 10^{-4}$--$1$ s$^{-1}$ and fitting the results gives
