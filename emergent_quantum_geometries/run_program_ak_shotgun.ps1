# run_program_ak_shotgun.ps1  --  Program AK TPU Orchestrator
# Project: time-emission
# Target: emergent_quantum_geometries/program_ak_predictive_sweep.py

$Project   = "time-emission"
$SrcDir    = "$PSScriptRoot"
$RemoteDir = "/home/cityz/program_ak"
...
$QRName  = "program-aj-qr-v6"
$NodeId  = "program-aj-node-v6"
$Zone    = "europe-west4-a"
$Type    = "v6e-8"
...
while (-not $success) {
    ...
    "y" | gcloud compute tpus tpu-vm ssh $NodeId `
        --project=$Project --zone=$Zone `
        --command="mkdir -p $RemoteDir/emergent_quantum_geometries/program_ak_results && pip install -q -U 'jax[tpu]' scipy numpy -f https://storage.googleapis.com/jax-releases/libtpu_releases.html"
    
    # 5. SCP Files (Using tar to avoid wildcard issues on Windows)
    Write-Host "[SCP] Uploading code files..."
    cd $SrcDir
    tar.exe -czf "payload_ak.tar.gz" *.py
    "y" | gcloud compute tpus tpu-vm scp payload_ak.tar.gz "$NodeId`:$RemoteDir`/emergent_quantum_geometries`/" --project=$Project --zone=$Zone 2>&1 | Out-Null
    "y" | gcloud compute tpus tpu-vm ssh $NodeId --project=$Project --zone=$Zone --command="cd $RemoteDir/emergent_quantum_geometries && tar -xzf payload_ak.tar.gz && rm payload_ak.tar.gz" 2>&1 | Out-Null
    Remove-Item -Force "payload_ak.tar.gz" -ErrorAction SilentlyContinue
    Write-Host "[SCP] Upload complete."

    Write-Host "[RESTORE] Uploading previously computed checkpoints..."
    if (Test-Path "$SrcDir\program_ak_results") {
        tar.exe -czf "results_ak.tar.gz" program_ak_results
        "y" | gcloud compute tpus tpu-vm scp results_ak.tar.gz "$NodeId`:$RemoteDir`/emergent_quantum_geometries`/" --project=$Project --zone=$Zone 2>&1 | Out-Null
        "y" | gcloud compute tpus tpu-vm ssh $NodeId --project=$Project --zone=$Zone --command="cd $RemoteDir/emergent_quantum_geometries && tar -xzf results_ak.tar.gz && rm results_ak.tar.gz" 2>&1 | Out-Null
        Remove-Item -Force "results_ak.tar.gz" -ErrorAction SilentlyContinue
    }

    # 6. Execute Experiment
    Write-Host "[EXECUTE] Launching Forecast Horizon & Stability Phase Diagram experiment..."
    $RunCmd = "export OMP_NUM_THREADS=64; cd $RemoteDir/emergent_quantum_geometries; nohup python3 -u program_ak_predictive_sweep.py --require-tpu > $LogFile 2>&1 &"
    "y" | gcloud compute tpus tpu-vm ssh $NodeId `
        --project=$Project --zone=$Zone `
        --command=$RunCmd
    ...
    while ((Get-Date) -lt $RunTimeout) {
        ...
        New-Item -ItemType Directory -Force -Path "$SrcDir\program_ak_results" | Out-Null
        "y" | gcloud compute tpus tpu-vm scp --recurse "$NodeId`:$RemoteDir`/emergent_quantum_geometries`/program_ak_results`/*" "$SrcDir\program_ak_results\" --project=$Project --zone=$Zone --quiet 2>$null
