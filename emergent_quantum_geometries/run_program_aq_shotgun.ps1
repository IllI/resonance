# run_program_aq_shotgun.ps1  --  Program AQ TPU Orchestrator
# Project: time-emission
# Target: emergent_quantum_geometries/kernel/program_aq_so3_kernel_galerkin.py

$Project   = "time-emission"
$SrcDir    = "$PSScriptRoot"
$RemoteDir = "/home/cityz/program_aq"
$LogFile   = "prog_aq_run.log"
...
# Target config (v6e-8 in europe-west4-a)
$QRName  = "program-aq-qr-v6"
$NodeId  = "program-aq-node-v6"
$Zone    = "us-east1-d"
$Type    = "v6e-8"
...
function Invoke-TpuSsh {
    ...
    gcloud compute tpus tpu-vm ssh $NodeId `
        --project=$Project `
        --zone=$Zone `
...
function Invoke-TpuScp {
    ...
    gcloud compute tpus tpu-vm scp $recurseFlag `
        $LocalPath "$NodeId`:$RemotePath" `
        --project=$Project `
        --zone=$Zone `
...
while (-not $success) {
    ...
    while (-not $sshReady -and (Get-Date) -lt $sshTimeout) {
        Invoke-TpuSsh "echo SSH_READY" 2>&1 | Out-Null
    ...
    Invoke-TpuSsh "mkdir -p $RemoteDir/emergent_quantum_geometries/program_aq_results && pip install -q -U 'jax[tpu]' scipy numpy -f https://storage.googleapis.com/jax-releases/libtpu_releases.html"
    
    # 5. SCP Files (Using tar to avoid wildcard issues on Windows)
    Write-Host "[SCP] Uploading code files..."
    cd $SrcDir
    tar.exe -czf "payload_aq.tar.gz" *.py kernel/program_aq_so3_kernel_galerkin.py
    Invoke-TpuScp payload_aq.tar.gz "$RemoteDir/emergent_quantum_geometries/" 2>&1 | Out-Null
    Invoke-TpuSsh "cd $RemoteDir/emergent_quantum_geometries && tar -xzf payload_aq.tar.gz && rm payload_aq.tar.gz" 2>&1 | Out-Null
    Remove-Item -Force "payload_aq.tar.gz" -ErrorAction SilentlyContinue
    Write-Host "[SCP] Upload complete."

    # 6. Execute Experiment
    Write-Host "[EXECUTE] Launching Program AQ SO(3) Galerkin sweep..."
    $RunCmd = "export OMP_NUM_THREADS=64; cd $RemoteDir/emergent_quantum_geometries; nohup python3 -u kernel/program_aq_so3_kernel_galerkin.py --n-centers 500 1000 2000 5000 10000 --N-truncation 5 10 15 20 --oversampling-ratios 10 > $LogFile 2>&1 &"
    Invoke-TpuSsh $RunCmd
    ...
    while ((Get-Date) -lt $RunTimeout) {
        ...
        $tail = Invoke-TpuSsh "tail -n 15 $RemoteDir/emergent_quantum_geometries/$LogFile 2>/dev/null" 2>$null
        Write-Host "[$([math]::Round(((Get-Date)-$StartTime).TotalMinutes,1))m] -------------------"
        Write-Host $tail
        
        # ATOMIC SYNC
        Write-Host "[SYNC] Pulling latest checkpoints..."
        New-Item -ItemType Directory -Force -Path "$SrcDir\program_aq_results" | Out-Null
        Invoke-TpuScp --recurse "$RemoteDir/emergent_quantum_geometries/program_aq_results/*" "$SrcDir\program_aq_results\" --project=$Project --zone=$Zone 2>&1 | Out-Null
        
        # Check if complete
        $doneRaw = Invoke-TpuSsh "grep -c 'Results written to' $RemoteDir/emergent_quantum_geometries/$LogFile 2>/dev/null || echo 0" 2>$null
        ...
        $heartbeat = Invoke-TpuSsh "cat $RemoteDir/emergent_quantum_geometries/heartbeat.txt 2>/dev/null || echo 0" 2>$null
        $hbVal = if ($heartbeat -is [array]) { $heartbeat[-1] } else { $heartbeat }
        
        if ($hbVal -ne $null -and [double]($hbVal.Trim()) -gt 0) {
            $currentTime = [double](Invoke-TpuSsh "date +%s" 2>$null | Select-Object -Last 1)
            ...
            if ($diff -gt 180) {
                ...
                $errorLog = Invoke-TpuSsh "tail -n 50 $RemoteDir/emergent_quantum_geometries/$LogFile" 2>$null
                Write-Host $errorLog
                break
            }
        }
