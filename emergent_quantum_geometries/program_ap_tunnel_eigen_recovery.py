        ...
        if args.run_semantic_recovery:
            ...
            for noise_idx, noise_level in enumerate(args.recovery_noise_sweep):
                ...
                for seed in args.recovery_seeds:
                    seed_key = jax.random.PRNGKey(int(seed))
                    depth_record["semantic_recovery"][str(noise_level)][str(seed)] = {}
                    print(f"      seed={seed}")
                    for ctrl_idx, (ctrl_name, cfg) in enumerate(controllers.items()):
                    ...
                    partial = {
                        "experiment": (
                            "tunnel_eigen_adaptive_alpha_ap_t"
                            if getattr(args, "ap_t", False)
                            else (
                                "tunnel_eigen_stability_fidelity_ap_s_alpha_bracket"
                                if getattr(args, "ap_s_fidelity", False)
                                else (
                                    "tunnel_eigen_stability_fidelity_ap_s_mech"
                                    if getattr(args, "ap_s_mech", False)
                                    else (
                                        "tunnel_eigen_stability_fidelity_ap_s"
                                        if getattr(args, "ap_s", False)
                                        else (
                                            "tunnel_eigen_residual_folded_recovery_ap_r"
                                            if getattr(args, "ap_r", False)
                                            else (
                                                "tunnel_eigen_guided_folded_recovery_ap_lite"
                                                if getattr(args, "ap_lite", False)
                                                else "tunnel_eigen_guided_folded_recovery"
                                            )
                                        )
                                    )
                                )
