...
def make_predictive_runner(block_pairs, N, n_steps, bob_gates, lambdas_j, d_hidden):
    def _run(evals, evecs, dt, p1, p2, p_cross, T_quiet, eps_predictive, forecast_horizon, psi0, rng_key, K, rnn_params):
        ...
        def step(carry, s):
            ...
            return (psi, S_curr, ssl_n, ng_n, key, z_curr, h_next_1), (fired, pred_drift)
    ...
    return jax.jit(jax.vmap(_run, in_axes=(None, None, None, None, None, None, None, None, None, 0, 0, None, None)))
...
def run_program_aj(args):
    ...
    print("=" * 80)
    print("Program AJ — Forecast Horizon Sweep")
    print("=" * 80)
    ...
    summary_path = os.path.join(args.out_dir, "program_aj_summary.json")
    all_records = []
    ...
    for depth in args.depth_sweep:
        ...
        for seed in seeds:
            ...
            pmap_predictive = jax.pmap(vmap_predictive, in_axes=(None, None, None, None, None, None, None, None, None, 0, 0, None, None))
            ...
            for sl in seq_lengths:
                ...
                def run_ensembles_parallel(evals_h, evecs_h, ctrl_type="free", k_horiz=1, K=None, rnn_params=None):
                    rhos_all = []
                    gates_all = []
                        ...
                        for sym_t in range(sl):
                            ...
                            if ctrl_type == "free":
                                ...
                                psi_batch, ng, *_ = pmap_predictive(
                                    ...
                                    jnp.int32(k_horiz), psi_batch, traj_keys_batch, K, rnn_params)
                    ...
                    for ctrl, k_horiz, ctrl_name, npz_path in to_run:
                        ...
                        try:
                            with open(summary_path, "w") as f:
                                json.dump({"experiment": "forecast_horizon_sweep", "records": all_records_temp + [rec_temp], "runtime_s": round(time.time() - t0, 1)}, f, indent=2)
                        except:
                            pass
    ...
    out = {
        "experiment": "forecast_horizon_sweep",
        "seed_suite": seeds,
        "records": all_records,
...
if __name__ == "__main__":
    ...
    p.add_argument("--out-dir", default="program_aj_results")
    ...
    p.add_argument("--k-horizons", nargs="+", type=int, default=[1, 2, 4, 8])
    args = p.parse_args()
