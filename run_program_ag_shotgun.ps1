# run_program_ag_shotgun.ps1  --  Program AG TPU Orchestrator
# Project: time-emission
# Target: emergent_quantum_geometries/program_ag_tpu.py

$Project   = "time-emission"
$SrcDir    = "$PSScriptRoot\emergent_quantum_geometries"
$RemoteDir = "/home/cityz/program_ag"
...
$RunCmd = "export OMP_NUM_THREADS=64; nohup python3 -u $RemoteDir/emergent_quantum_geometries/program_ag_tpu.py --require-tpu > $RemoteDir/$LogFile 2>&1 &"
"y" | gcloud compute tpus tpu-vm ssh $NodeId `
    --project=$Project --zone=$Zone `
    --command=$RunCmd
...
while ((Get-Date) -lt $RunTimeout) {
    ...
    $isRunning = "y" | gcloud compute tpus tpu-vm ssh $NodeId `
        --project=$Project --zone=$Zone `
        --command="pgrep -f program_ag_tpu.py >/dev/null && echo 1 || echo 0" 2>$null
...
$remoteResult = "$NodeId`:$RemoteDir`/program_ag_results`/program_ag_summary.json"
$remoteLog    = "$NodeId`:$RemoteDir`/$LogFile"

New-Item -ItemType Directory -Force -Path "$SrcDir\program_ag_results" | Out-Null
"y" | gcloud compute tpus tpu-vm scp $remoteResult "$SrcDir\program_ag_results\program_ag_summary.json" --project=$Project --zone=$Zone
"y" | gcloud compute tpus tpu-vm scp $remoteLog "$SrcDir\program_ag_results\program_ag_run.log" --project=$Project --zone=$Zone
