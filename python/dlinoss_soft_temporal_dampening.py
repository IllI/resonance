...
class TemporalDampeningAttention(nn.Module):
    def __init__(self, target_dim):
        super().__init__()
        # Learned Temporal Attention. Instead of a hard slice, the model LEARNS 
        # to "zoom in", "slow down", and "amplify" the signal exactly when the Ghost forms,
        # while dampening the motor response and mind wandering post-decision.
    ...
    def forward(self, x_seq):
        ...
        attn_scores = self.attn_net(x_seq) # (B, T, 1)
        
        # We apply softmax to create a normalized weighting curve across time
        attn_weights = F.softmax(attn_scores, dim=1) 
        
        # Amplify and dampen the raw mapped output dynamically
        context_vector = torch.sum(attn_weights * x_seq, dim=1) # (B, Dim)
...
class DLinOSSSuperpositionMapper(nn.Module):
    def __init__(self, in_dim=1000, target_dim=512, dlinoss_state_dim=32, dlinoss_hidden=64):
        ...
        # Using sophisticated sequence processing
        self.temporal_attn = TemporalDampeningAttention(target_dim)
        
    def forward(self, x_seq):
        # 1. Map physical EM waves
        x_oscillating = self.dlinoss(x_seq)
        # 2. Scrub overlapping features
        sparse_features, dlinoss_recon = self.sae(x_oscillating)
        # 3. Map to semantic space
        x_mapped = self.semantic_map(sparse_features)
        
        # 4. Apply the Soft Temporal Dampening (Zoom in, amplify, mute motor response)
        event_vector, attn_weights = self.temporal_attn(x_mapped)
        
        return F.normalize(event_vector.squeeze(0), p=2, dim=0), sparse_features, dlinoss_recon, attn_weights

def run_dampened_hyperalignment():
    ...
    # WE DO NOT HARD CROP. 
    # We pass the full 10 TR block (~25 seconds) allowing the model to 
    # zoom in on the initial structural pathway and dampen the noise.
    train_events = [ev for ev in all_events if ev[0] != test_subject]
    test_events = [ev for ev in all_events if ev[0] == test_subject]
    
    print(f"\n--- Fortifying Model with Temporal Attention on {len(train_events)} uncropped events ---")
    ...
    for ep in range(30):
        ...
        for subj, cat, x_ev, _ in train_events:
            opt.zero_grad()
            # Feed the full sequence
            x_ev_tensor = torch.tensor(x_ev, dtype=torch.float32).unsqueeze(0)
            pred_vec, z_features, x_recon, attn_weights = model(x_ev_tensor)
            with torch.no_grad():
    ...
    # Let's peek at the extracted timeline to see what it zoomed in on!
    print("Example Temporal Dampening Curve for a 'Face' Block:")
    face_example = [ev for ev in test_events if ev[1] == 'face'][0]
    ...
    with torch.no_grad():
        for subj, true_cat, x_ev, _ in test_events:
            ...
            if true_cat == 'face' and subj == test_subject and 'print_once' not in locals():
                w = attn_weights[0].cpu().numpy()
                print("TR (2.5s intervals):", [f"T{i}" for i in range(len(w))])
                print("Attention Weight:   ", [f"{val:.3f}" for val in w])
                print_once = True
