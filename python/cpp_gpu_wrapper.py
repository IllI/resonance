#!/usr/bin/env python3
"""
C++ GPU Wrapper - Python wrapper for C++ GPU control
This uses the C++ DLL to force discrete GPU usage
"""
...
class CppGPUWrapper:
    """Python wrapper for C++ GPU control"""
    ...
    def load_dll(self):
        """Load the C++ GPU control DLL"""
        try:
            # Try to load the compiled DLL
            dll_path = Path(__file__).parent / "cpp_gpu_control.dll"
            if dll_path.exists():
                self.dll = ctypes.CDLL(str(dll_path))
                print(" Loaded C++ GPU Control DLL")
                self.setup_dll_functions()
            else:
                print("  C++ GPU Control DLL not found")
                print("   Compile cpp_gpu_control.cpp first:")
                print("   cl /LD cpp_gpu_control.cpp")
    ...
    def test_gpu_usage(self):
        ...
        print("\n Testing GPU usage with C++ control...")
...
def compile_cpp_dll():
    """Compile the C++ DLL"""
    print("🔨 Compiling C++ GPU Control DLL...")
    
    try:
        import subprocess
        
        # Compile the C++ file
        cmd = ['cl', '/LD', 'cpp_gpu_control.cpp']
        result = subprocess.run(cmd, capture_output=True, text=True)
        
        if result.returncode == 0:
            print(" C++ DLL compiled successfully")
            print("   Created: cpp_gpu_control.dll")
        else:
        ...
        print("   Or manually compile: cl /LD cpp_gpu_control.cpp")

def main():
    """Main function"""
    print("=== C++ GPU Wrapper ===")
    print("Using C++ DLL to force discrete GPU usage")
    
    # Try to compile DLL if it doesn't exist
    dll_path = Path(__file__).parent / "cpp_gpu_control.dll"
    if not dll_path.exists():
        print(" C++ GPU Control DLL not found, attempting to compile...")
        compile_cpp_dll()
    
    # Create wrapper
    wrapper = CppGPUWrapper()
    ...
    print("   - The C++ approach may not be sufficient")
    print("   - Consider manual Windows Graphics Settings")
    ...
    print("="*60)
    print("from cpp_gpu_wrapper import CppGPUWrapper")
    print("")
    print("# Initialize C++ GPU control")
    print("gpu_wrapper = CppGPUWrapper()")
    print("gpu_wrapper.request_high_performance_gpu()")
    print("")
