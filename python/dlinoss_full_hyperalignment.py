...
def run_fortified_hyperalignment():
    ...
    # ISOLATING THE GHOST PROCESSING WINDOW
    # TR 1 to 4 is roughly 2.5s to 10s (adjusting for HRF shape)
    ...
    # Test subject is sub-5. Everyone else is train.
    test_subject = 'sub-5'
    train_events = [ev for ev in sliced_events if ev[0] != test_subject]
    test_events = [ev for ev in sliced_events if ev[0] == test_subject]
    
    print(f"\n--- Fortifying Model: Training on ALL available subjects except {test_subject} ---")
    print(f"Total training events: {len(train_events)}. Temporally isolated Ghost window.")
    ...
    print("\n--- Unsupervised Representational Similarity Hyperalignment (RSA) ---")
    print(f"{test_subject} is completely completely unanalyzed. Extracting their rotated Ghost Manifold...")
    model.eval()
    test_vecs = []
    test_labels = []
    ...
    print("Calculating Internal Geometry Matrices (Representational Similarity)...")
    sub5_sim = 1 - squareform(pdist(sub5_centroids, metric='cosine'))
    anchor_sim = 1 - squareform(pdist(anchor_matrix, metric='cosine'))
    
    print("Executing RSA Bipartite Shape Matching (Graph Matching)...")
    # Finding the permutation that optimally maps sub5 geometry to anchor geometry
    res = quadratic_assignment(sub5_sim, anchor_sim, options={'maximize': True})
    col_ind = res.col_ind
    
    print("Matched Internal Concept Structure! Computing Procrustes Rotation...")
    matched_anchors = anchor_matrix[col_ind]
    matched_sub5_centroids = sub5_centroids # since we map sub5_centroids[i] to matched_anchors[i]
    
    R, scale = orthogonal_procrustes(matched_sub5_centroids, matched_anchors)
    print("Successfully mapped and rotated unanalyzed subject's inner brain structure to Universal Basin!")
    ...
    correct_mountains = 0
    print(f"\n--- Evaluating {test_subject}'s ZERO-SHOT Generalization ---")
