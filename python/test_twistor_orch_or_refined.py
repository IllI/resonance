...
def test_twistor_model():
    ...
    print("\n--- TCS by Phase & Correctness ---")
    for phase in ['Exploration', 'Pre-Grokking', 'Grokking', 'Expert']:
        pts_c1 = [t['TCS'] for t in global_timeline if t['Phase'] == phase and t['Correct'] == 1]
        pts_c0 = [t['TCS'] for t in global_timeline if t['Phase'] == phase and t['Correct'] == 0]
        
        print(f"Phase: {phase}")
        print(f"  Correct (C=1):   n={len(pts_c1):<3} mean_TCS={np.mean(pts_c1) if pts_c1 else 0:.4f}")
        print(f"  Incorrect (C=0): n={len(pts_c0):<3} mean_TCS={np.mean(pts_c0) if pts_c0 else 0:.4f}")
