...
def check_correlations():
    ...
    print("==========================================================================")
    print(" ANALYZING BEHAVIORAL CORRELATIONS FOR EXPERT VS EXPLORATION")
    print("==========================================================================")
    ...
    expert_trials = [t for t in valid_trials if t['Phase'] == 'Expert']
    exploration_trials = [t for t in valid_trials if t['Phase'] == 'Exploration']
    
    print("\n[EXPERT PHASE]")
    correlation_matrix_e = np.zeros((len(vars_to_correlate), len(vars_to_correlate)))
    for i, var1 in enumerate(vars_to_correlate):
        for j, var2 in enumerate(vars_to_correlate):
            v1_data = [t[var1] for t in expert_trials]
            v2_data = [t[var2] for t in expert_trials]
            corr, _ = pearsonr(v1_data, v2_data)
            correlation_matrix_e[i, j] = corr

    df_corr_e = pd.DataFrame(correlation_matrix_e, index=vars_to_correlate, columns=vars_to_correlate)
    print(df_corr_e.to_string(float_format=lambda x: f"{x:.3f}"))

    print("\n[EXPLORATION PHASE]")
    correlation_matrix_ex = np.zeros((len(vars_to_correlate), len(vars_to_correlate)))
    for i, var1 in enumerate(vars_to_correlate):
        for j, var2 in enumerate(vars_to_correlate):
            v1_data = [t[var1] for t in exploration_trials]
            v2_data = [t[var2] for t in exploration_trials]
            corr, _ = pearsonr(v1_data, v2_data)
            correlation_matrix_ex[i, j] = corr

    df_corr_ex = pd.DataFrame(correlation_matrix_ex, index=vars_to_correlate, columns=vars_to_correlate)
    print(df_corr_ex.to_string(float_format=lambda x: f"{x:.3f}"))
