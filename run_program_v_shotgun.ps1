<#
.SYNOPSIS
  run_program_v_shotgun.ps1  --  Program V TPU launcher (Bible-compliant)
  Queued Resource API only. Correct delete ritual. gcloud native SSH auto-accept.
  Project: time-emission
  Target: program_v_tpu.py (N=12 by default for 3x4 grid)
#>
...
param(
    ...
    [string]$Controllers = "static dd random vector_adaptive",
    [string]$TMax        = "8.0",
    [string]$NSteps      = "40",
    [string]$B           = "3",
    [string]$NTraj       = "100",
    ...
    [int]   $QueueMaxMin = 30,
    [int]   $MaxMin      = 240,
    [string]$OutDir      = "program_v_results"
)

$Project   = "time-emission"
$SrcDir    = "$PSScriptRoot\emergent_quantum_geometries"
$RemoteDir = "/home/cityz/program_v"
$LogFile   = "prog_v_run.log"
...
foreach ($file in @("program_v_tpu.py", "program_t_tpu.py", "program_l_tpu.py", "program_s1_tpu.py")) {
...
Write-Host "[RUN] Launching Program V (nohup background)..."
"y" | gcloud compute tpus tpu-vm ssh $($Winner.NodeId) `
    --project=$Project --zone=$($Winner.Zone) `
    --command=$RunCmd
...
$remoteResult = $Winner.NodeId + ':' + $RemoteDir + '/' + $OutDir + "/program_v_N${N}_results.json"
$srcLog     = $Winner.NodeId + ':' + $RemoteDir + '/' + $LogFile
New-Item -ItemType Directory -Force -Path "$SrcDir\$OutDir" | Out-Null
"y" | gcloud compute tpus tpu-vm scp $remoteResult "$SrcDir\$OutDir\program_v_N${N}_results.json" `
    --project=$Project --zone=$($Winner.Zone)
