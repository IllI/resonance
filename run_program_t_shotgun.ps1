<#
.SYNOPSIS
  run_program_t_shotgun.ps1  --  Program T TPU launcher (Bible-compliant)
  Queued Resource API only. Correct delete ritual. gcloud native SSH auto-accept.
  Project: time-emission
  Target: program_t_tpu.py (N=12 by default for 3x4 grid)
#>

param(
    [int]$Lx             = 3,
    [int]$Ly             = 4,
    [string]$Models      = "GridXXZ_uniform GridXXZ_hetero GridXXZ_pockets GridXXZ_protected",
    [string]$Controllers = "static dd random sparse_predictive probe_sparse",
    [string]$TMax        = "8.0",
    [string]$NSteps      = "40",
    [string]$B           = "3",
    [string]$Seed        = "42",
    [int]   $QueueMaxMin = 30,
    [int]   $MaxMin      = 240,
    [string]$OutDir      = "program_t_results"
)

$Project   = "time-emission"
$SrcDir    = "$PSScriptRoot\emergent_quantum_geometries"
$RemoteDir = "/home/cityz/program_t"
$LogFile   = "prog_t_run.log"
...
$Candidates = @(
    @{ QRName="prog-t-v4od";  NodeId="prog-t-v4od-node";  Zone="us-central2-b";  Type="v4-8";         Runtime="tpu-vm-v4-base";      Flag="" },
    @{ QRName="prog-t-v6e1";  NodeId="prog-t-v6e1-node";  Zone="us-east1-d";     Type="v6e-8";        Runtime="v2-alpha-tpuv6e";     Flag="--spot" },
    @{ QRName="prog-t-v6e2";  NodeId="prog-t-v6e2-node";  Zone="europe-west4-a"; Type="v6e-8";        Runtime="v2-alpha-tpuv6e";     Flag="--spot" },
    @{ QRName="prog-t-v5e1";  NodeId="prog-t-v5e1-node";  Zone="europe-west4-b"; Type="v5litepod-4";  Runtime="v2-alpha-tpuv5-lite"; Flag="--spot" },
    @{ QRName="prog-t-v5e2";  NodeId="prog-t-v5e2-node";  Zone="us-central1-a";  Type="v5litepod-4";  Runtime="v2-alpha-tpuv5-lite"; Flag="--spot" },
    @{ QRName="prog-t-v4s";   NodeId="prog-t-v4s-node";   Zone="us-central2-b";  Type="v4-8";         Runtime="tpu-vm-v4-base";      Flag="--spot" }
)
...
while (-not $sshReady -and (Get-Date) -lt $sshDeadline) {
    ...
    # User feedback: make sure to auto upload ssh and hit 'y' when prompted by gcloud
...
foreach ($file in @("program_t_tpu.py", "program_l_tpu.py", "program_s1_tpu.py")) {
...
Write-Host "[RUN] Launching Program T (nohup background)..."
"y" | gcloud compute tpus tpu-vm ssh $($Winner.NodeId) `
    --project=$Project --zone=$($Winner.Zone) `
    --command=$RunCmd
...
$remoteResult = $Winner.NodeId + ':' + $RemoteDir + '/' + $OutDir + "/program_t_N${N}_results.json"
$srcLog     = $Winner.NodeId + ':' + $RemoteDir + '/' + $LogFile
New-Item -ItemType Directory -Force -Path "$SrcDir\$OutDir" | Out-Null
"y" | gcloud compute tpus tpu-vm scp $remoteResult "$SrcDir\$OutDir\program_t_N${N}_results.json" `
    --project=$Project --zone=$($Winner.Zone)
"y" | gcloud compute tpus tpu-vm scp $srcLog "$SrcDir\$OutDir\$LogFile" `
    --project=$Project --zone=$($Winner.Zone) 2>$null

Delete-All @($Winner)
Write-Host "[COMPLETE] Program T shotgun done."
