# run_program_al_shotgun.ps1  --  Program AG TPU Orchestrator
# Project: time-emission
# Target: emergent_quantum_geometries/program_al_semantic_modes.py

$Project   = "time-emission"
$SrcDir    = "$PSScriptRoot\emergent_quantum_geometries"
$RemoteDir = "/home/cityz/program_al"
$LogFile   = "prog_ag_run.log"
...
$QRName  = "program-ag-qr-v4"
$NodeId  = "program-ag-node-v4"
$Zone    = "europe-west4-a"
$Type    = "v6e-8"
...
$RunCmd = "export OMP_NUM_THREADS=64; nohup python3 -u $RemoteDir/emergent_quantum_geometries/program_al_semantic_modes.py --require-tpu > $RemoteDir/$LogFile 2>&1 &"
"y" | gcloud compute tpus tpu-vm ssh $NodeId `
    --project=$Project --zone=$Zone `
    --command=$RunCmd
...
while ((Get-Date) -lt $RunTimeout) {
    ...
    $doneRaw = "y" | gcloud compute tpus tpu-vm ssh $NodeId `
        --project=$Project --zone=$Zone `
        --command="grep -c 'Checkpoint saved ->' $RemoteDir/$LogFile 2>/dev/null || echo 0" 2>$null
    ...
    $isRunning = "y" | gcloud compute tpus tpu-vm ssh $NodeId `
        --project=$Project --zone=$Zone `
        --command="pgrep -f program_al_semantic_modes.py >/dev/null && echo 1 || echo 0" 2>$null
...
$remoteResult = "$NodeId`:$RemoteDir`/program_al_results`/program_al_summary.json"
$remoteLog    = "$NodeId`:$RemoteDir`/$LogFile"

New-Item -ItemType Directory -Force -Path "$SrcDir\program_al_results" | Out-Null
"y" | gcloud compute tpus tpu-vm scp $remoteResult "$SrcDir\program_al_results\program_al_summary.json" --project=$Project --zone=$Zone
"y" | gcloud compute tpus tpu-vm scp $remoteLog "$SrcDir\program_al_results\program_al_run.log" --project=$Project --zone=$Zone
